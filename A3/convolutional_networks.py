...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros((N, F, H_, W_)).to(x)

        # this part performs the act of doing the convolutional process
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad))
        for i in range(H_):
            for j in range(W_):
                x_pad_ = x_pad[:, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                for k in range(F):
                    out[:, k, i, j] = torch.sum(x_pad_ * w[k, :, :, :], dim = (1, 2, 3))
    ...
    def backward(dout, cache):
        ...
        for i in range(H_):
            for j in range(W_):
                x_pad_ = x_pad[:, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                for k in range(F):
                    dw[k, :, :, :] += torch.sum(x_pad_ * dout[:, k, i, j].view(-1, 1, 1, 1), dim = 0)
                for n in range(N):
                    dx_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride + WW] += torch.sum(w[:, :, :, :] * dout[n, :, i, j].view(-1, 1, 1, 1), dim = 0)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.tensor(N, C, H_out, W_out).to(x)
        
        for i in range(H_out):
            for j in range(W_out):
                out[:, :, i, j] = torch.amax(x[:, :, i * stride:i * stride + pool_height, j * stride:j * stride + pool_width])
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                max = torch.amax(x_masked, dim = (2, 3)).view(N, -1, 1, 1)
                mask_max = (x_masked == max)
                dx[:, :, i * stride:i * stride + pool_height, j * stride:j * stride + pool_width] += mask_max * dout[:, :, i, j].view(N, -1, 1, 1)
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # first use convolutional neural network, then use Linear_ReLU, then use LInear
        # conbine together, form a 3 layer network
        conv_forward, cache_conv = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        linear_forward, cache_linear = Linear_ReLU.forward(conv_forward, W2, b2)
        scores, cache_final = Linear.forward(linear_forward, W3, b3)
        ...
        dx3, grads['W3'], grads['b3'] = Linear.backward(dout, cache_final)
        dx2, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx3, cache_linear)
        dx1, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx2, cache_conv)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        H_, W_ = filter_size, filter_size

        for i, num_filter in enumerate(num_filters):
            H_out = 1 + (H_out + 2 * conv_param['pad'] - H_) // conv_param['stride']
            W_out = 1 + (W_out + 2 * conv_param['pad'] - W_) // conv_param['stride']

            if self.batchnorm:
                self.params['gama%d'%i] = 0.01 * torch.randn(num_filter, device = device, dtype = dtype)
                self.params['beta%d'%i] = 0.01 * torch.randn(num_filter, divice = device, dtype = dtype)
        ...
        if weight_scale == 'kaiming':
            ...
            self.params['W%d'%i] = weight_scale * torch.randn(num_filters * H_out * W_out, num_classes, dtype = dtype, device = device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if i in self.max_pools:
                if self.batchnorm:
                    out, cache['%d'%i] = Conv_BatchNorm_ReLU_Pool.forward(out, self.params['W%d'%i], self.params['b%d'%i], self.params['gamma%d'%i], self.params['beta%d'%i], conv_param, self.bn_params[i], pool_param)
                else:
                    out, cache['%d'%i] = Conv_ReLU_Pool.forward(out, self.params['W%d'%i], self.params['b%d'%i], conv_param, pool_param)
            else:
                if self.batchnorm:
                    out, cache['%d'%i] = Conv_BatchNorm_ReLU.forward(out, self.params['W%d'%i], self.params['b%d'%i], self.params['gamma%d'%i], self.params['beta%d'%i], conv_param, self.bn_params[i])
                else:
                    out, cache['%d'%i] = Conv_ReLU.forward(out, self.params['W%d'%i], self.params['b%d'%i], conv_param)
            
        i += 1
        out, cache['%d'%i] = Linear.forward(out, self.params['W%d'%i], self.params['b%d'%i])
        scores = out
        ...
        for i in range(self.num_layers):
            if self.batchnorm:
                if i <= (self.num_layers - 2):
                    loss += torch.sum(self.params['gamma%d'%i]**2) * self.reg
            loss += torch.sum(self.params['W%d'%i]**2) * self.reg

        dout, dw, db = Linear.backward(dout, cache['%d'%i])
        ...
        for i in range(i, -1, -1):
            if i in self.max_pools:
                if self.batchnorm:
                    dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, cache['%d'%i])
                    grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
                    grads['b%d'%i] = db
                    grads['beta%d'%i] = dbeta
                    grads['gamma%d'%i] = dgamma + 2 * self.reg * self.params['gamma%d'%i]
                else:
                    dout, dw, db = Conv_ReLU_Pool.backward(dout, cache['%d'%i])
                    grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
                    grads['b%d'%i] = db
            else:
                if self.batchnorm:
                    dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache['%d'%i])
                    grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
                    grads['b%d'%i] = db
                    grads['gamma%d'%i] = dgamma + 2 * self.reg * self.params['gamma%d'%i]
                    grads['beta%d'%i] = dbeta
                else:
                    dout, dw, db = Conv_ReLU.backward(dout, cache['%d'%i])
                    grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
                    grads['b%d'%i] = db        
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims = input_dims, num_classes = 10,
                        ...
                        dtype = dtype,
                        device = device)
    solver = Solver(model, data_dict, num_epochs = 50, batch_size = 128, update_rule = adam,
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            mu = 1.0/N * torch.sum(x, dim = 0)
            running_mean = momentum * running_mean + (1 - momentum) * mu
            xmu = x - mu
            var = 1.0/N * torch.sum(xmu ** 2, dim = 0)
