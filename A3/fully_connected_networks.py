...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        compute_tensor = x.view(N, -1) # now X is a tensor with shape (N, D)
        out = torch.mm(compute_tensor, w) + b
    ...
    def backward(dout, cache):
        ...
        # this give us the matric that dout is the upstream derivatives
        # use this to compute the gradient of all these dx, dw, db
        # using backpropogation

        N = x.shape[0]
        x_reshape = x.view(N, -1)
        dx = torch.mm(dout, w.t())
        dx = torch.reshape(dx, x.shape)
        dw = torch.mm(x_reshape.t(), dout) #dout (N, M) x_reshape (N, D)
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        # this compute the ReLU function
        # for those x has a value less than 0, the gradient gives zero, 
        # while others gives 1
        dx = dout * (x >= 0)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        # first let's compute the score using the giving dataset
        ...
        # compute the loss use the softmax (add L2 regularization) 
        loss, dscore = softmax_loss(scores, y)
        loss += self.reg * (torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(self.params['W2'] * self.params['W2']))

        # then let us compute the gradient 
        # use the function defined above to compute the gradient
        dx2, dw2, db2 = Linear.backward(dscore, cache_2)
        grads['W2'] = dw2 + 2 * self.reg * self.params['W2']
        grads['b2'] = db2

        dx1, dw1, db1 = Linear_ReLU.backward(dx2, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, next_dim in enumerate(hidden_dims):
            self.params['W%d'%(i + 1)] = weight_scale * torch.randn(cur_input_dim, next_dim, dtype = self.dtype, device = device)
            self.params['b%d'%(i + 1)] = weight_scale * torch.zeros(next_dim, dtype = dtype, device = device)
            cur_input_dim = next_dim
          
        self.params['W%d'%(self.num_layers)] = weight_scale * torch.randn(cur_input_dim, next_dim, dtype = self.dtype, device = device)
        self.params['b%d'%(self.num_layers)] = weight_scale * torch.zeros(next_dim, dtype = dtype, device = device)
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        # Replace "pass" statement with your code
        # how will I compute loss?
        # this part I will first give score
        # compu_time = self.num_layers
        cur_score = X
        cache = {}
        # this is add later
        drop_cache = {}
        # this part compute the scores of each layer
        for i in range(self.num_layers - 1):
            w = self.params['W%d'%(i + 1)]
            b = self.params['b%d'%(i + 1)]
            cur_score, cache[i] = Linear_ReLU.forward(cur_score, w, b)

            if self.use_dropout:
                cur_score, drop_cache[i] = Dropout.forward(cur_score, self.dropout_param)

        w = self.params['W%d'%(self.num_layers)]
        b = self.params['b%d'%(self.num_layers)]
        cur_score, cache[self.num_layers] = Linear.forward(cur_score, w, b)
        scores = cur_score
        """
        layer_input = X
        ar_cache = {}
        ...
        #####################################################################
        # Replace "pass" statement with your code
        # this part we will compute the loss and the gradient 
        loss, dscore = softmax_loss(scores, y)
        loss += self.reg * torch.sum(self.params['W%d'%(self.num_layers)]**2)
        dx, dw, db = Linear.backward(dscore, cache[self.num_layers])
        grads['W%d'%(self.num_layers)] = dw + 2 * self.reg * self.params['W%d'%(self.num_layers)]
        grads['b%d'%(self.num_layers)] = db

        dscore = dx
        for i in range(self.num_layers - 1):
            i_real = self.num_layers - 1 - i
            loss += self.reg * torch.sum(self.params['W%d'%(i_real)]**2)

            # this is add later
            if self.dropout_param:
                dscore = Dropout.backward(dscore, drop_cache[i_real-1])
            dx, dw, db = Linear_ReLU.backward(dscore, cache[i_real-1])
            grads['W%d'%(i_real)] = dw + 2 * self.reg * self.params['W%d'%(i_real)]
            grads['b%d'%(i_real)] = db
            dscore = dx
        """
        regu_loss = 0.0
        for i in range(self.num_layers):
            regu_loss += self.reg * torch.sum(self.params['W%i'%(i + 1)] ** 2)
        loss = score_loss + regu_loss
        i += 1
        dx, dw, db = Linear.backward(dscore, cache['%d'%i])
        grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
        grads['b%d'%i] = db
        dscore = dx
        for i in range(self.num_layers - 1):
            cur_index = self.num_layers - i - 1
            dx, dw, db = Linear_ReLU.backward(dscore, cache['%d'%cur_index])
            grads['W%d'%i] = dw + 2 * self.reg * self.params['W%d'%i]
            grads['b%d'%i] = db
            dscore = dx
        """
